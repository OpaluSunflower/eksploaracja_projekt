...
Do dalszego przetwarzania danych i ewlauacji modelu można wyrzucić kolumnę z indeksami krotek. Zbiór samochodów ograniczymy też do 10 najpopularniejszych marek (jednym z powodów jest to, że RandomForest nie można utworzyć jeżeli jest więcej niż ok 50. kategorii w kolumnie a tak będzie w przypadku chociażby Model Pojazdu)

```{r}
autka_dt <- subset(autka_dt,select = -id)
autka_dt$Marka_pojazdu <- factor(autka_dt$Marka_pojazdu, levels = c("Volvo","Audi","BMW","Volkswagen","Opel","Renault","Ford","Toyota","Mercedes-Benz","Citroën","Inny")) 
autka_dt <- na.omit(autka_dt)
head(autka_dt)
```
...
```{r}
sapply(autka_dt,class)
```
Ponieważ cecha "Model pojazdu" zawiere wiele unikalnych wartości będących wyznacznikami przynależności auta do określonej kategorii, usówamy kolumnę wartości ze zbioru danych.
```{r}
autka_dt <- subset(autka_dt,select=-Model_pojazdu)
```
...
autka_dt <- dummy_cols(autka_dt,select_columns = c("Marka_pojazdu","Rodzaj_paliwa","Typ_nadwozia","Kolor"),remove_selected_columns = TRUE)
```


RandomForest ma problem z "trudnymi nazwami". Dlatego zamienimy nazwy kolumn na takie, które nie powinny sprawiać problemów

```{r}
autka_dt$Marka_pojazdu_Mercedes_Benz <- autka_dt$`Marka_pojazdu_Mercedes-Benz`
autka_dt <- subset(autka_dt,select=-`Marka_pojazdu_Mercedes-Benz`)
autka_dt$Typ_nadwozia_Auta_male <- autka_dt$`Typ_nadwozia_Auta małe`
autka_dt <- subset(autka_dt,select=-`Typ_nadwozia_Auta małe`)
autka_dt$Typ_nadwozia_Auta_miejskie <- autka_dt$`Typ_nadwozia_Auta miejskie`
autka_dt <- subset(autka_dt,select=-`Typ_nadwozia_Auta miejskie`)
autka_dt$Kolor_Inny_kolor <- autka_dt$`Kolor_Inny kolor`
autka_dt <- subset(autka_dt,select=-`Kolor_Inny kolor`)
autka_dt$Rodzaj_paliwa_Benzyna_LPG <- autka_dt$`Rodzaj_paliwa_Benzyna+LPG`
autka_dt <- subset(autka_dt,select=-`Rodzaj_paliwa_Benzyna+LPG`)
...
```{r}
autka_dt_no_eng_capacity <- subset(autka_dt,select = -Pojemność_skokowa)
autka_dt_no_eng_capacity

autka_dt_no_power <- subset(autka_dt,select = -Moc)
autka_dt_no_power
```
...
```{r}
indexes_train <- sample(1:nrow(autka_dt_no_eng_capacity), size= nrow(autka_dt_no_eng_capacity)*0.80)
...
```{r}
indexes_train <- sample(1:nrow(autka_dt_no_power), size= nrow(autka_dt_no_power)*0.80)
...
reg1_rmse <- RMSE(predict(Regresja1, newdata=Test_1), Test_1$Cena)
reg2_rmst <- RMSE(predict(Regresja2, newdata=Test_2), Test_2$Cena)
```
Random Forest

Zbadamy jakość tworzonych modeli w zależnośći od zmienianych parametrów. Podstawowym parametrem jest liczba drzew w lesie. Jakość będziemy badać dla liczby drzew z przedniału od 45 do 1000 co 5 (45,50,55,65,...,990,995,1000)


```{r}
  wyniki <- matrix(, ncol = 3)
```
...
for (x in 45:1000){
  if (x %% 5 == 0)
  {
  RF1 <- randomForest(Cena~., data=subset(Train_1),ntree=x,mtry=(ncol(autka_dt)-1)/3)
  pred1 <- RMSE(predict(RF1, newdata=Test_1), Test_1$Cena)
  RF2 <- randomForest(Cena~., data=subset(Train_2),ntree=x,mtry=(ncol(autka_dt)-1)/3)
  pred2 <- RMSE(predict(RF2, newdata=Test_2), Test_2$Cena)
  wyniki <- rbind(wyniki,c(x,pred1,pred2))
  }
}
wyniki <- wyniki[-c(1),]
wyniki
...
```{r}
w <- as.data.frame(wyniki)
colnames(w) <- c('n_tree', "Engine model RMSE","Capacity model RMSE")
ggplot()+ geom_line(data=w, aes(x=n_tree, y=`Engine model RMSE`), color = "blue") + geom_point()

  ggplot()+ geom_line(data=w, aes(x=n_tree, y=`Capacity model RMSE`), color = "red") +
  geom_point()
   
```
```{r}
summary(w)
reg_res <- data.frame(Model = c("Błąd średniokwadratowy dla regresji na zbiorze z mocą silnika", "Błąd średniokwadratowy dla regresji na zbiorze z pojemnością silnika"),Result =c(reg1_rmse,reg2_rmst))
reg_res
```

## 7. Podsumowanie

Na przetworzonym zbiorze danych zbudowano po dwa modele regresji logistycznej i drzewa decyzyjnego. Każdy algorytm uczenia mszynowego zastosowano na dwch zbiorach danych: jeden zawierał informacje o mocy silnika, drugi o pojemości silnika (rozdzielono te kolumny z powodu dużej korelacji)

Dla wszystkich modeli wyznaczono błąd średniokwadratowy. Można zauważyć, że dla zbioru z mocą silnika ów błąd jest mniejszy od błędu w modelach uczonych na zbiorze z pojemnością. Błąd średniokwadratowy we wszystkich przypadkach jest dosyć duży i prawdopodobnie wynika on ze specyfiki zbioru: mała liczba przykładów w stosunku do odchylenia danych i dużej ilości wartości kategorycznych.

Dla lasów losowych błędy średniokwadratowe były mniejsze niż dla odpowiadających modeli regresyjnych.

Zmiana podstawowego parametu jakim była liczba drzew w lesie losowym bezpośrednio nie polepszała jakości modeli. Nie można wskazać jednoznacznej korelacji między ilością drzew a wartością błędu średniokwadratowego dla lasów losowych.